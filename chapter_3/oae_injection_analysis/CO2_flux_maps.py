...
# Initialize figure
plt.close('all')
fig,ax = plt.subplots(1,1,figsize=(6,8.5), sharex=True, sharey=True) 
...
cs = ax.pcolormesh(px,py,diff,cmap=diff_cmap,vmin=0,vmax=0.003)#,norm=colors.LogNorm(vmin=1e-4,vmax=1e-1))
...
# ax[3].axis('off')
pfun.dar(ax[3])
ax.set_title(r'$\Delta$ CO$_2$ flux [mmol m$^{-2}$ d$^{-1}$]', fontsize=14)
# add injection location
ax.scatter(inj_lon, inj_lat, color='none', edgecolor='pink',marker='o', s=100,linewidth=3)
ax.scatter(inj_lon, inj_lat, color='none', edgecolor='crimson',marker='o', s=100,linewidth=2)
