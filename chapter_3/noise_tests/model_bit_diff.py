...
for i in np.linspace(1,2,2):
    if i < 10:
        oceanhis = 'ocean_his_000' +str(int(i)) + '.nc'
    else:
        oceanhis = 'ocean_his_00' +str(int(i)) + '.nc'

    save_dir = out_dir / ('hour' + str(i))
    Lfun.make_dir(save_dir)
    print('his file {}'.format(str(int(i))))
    ...
    for vn in vns:
        ...
        # Get model2 data
        if vn == 'dye_01':
            surf_vn_model2 = surf_vn_model1 * 0
            bott_vn_model2 = bott_vn_model1 * 0
        else:
        ...
        # plt.savefig(save_dir/'pcolormesh')
        # plt.close()
    ...
    #     plt.tight_layout()
    #     plt.savefig(save_dir/'residuals')
    #     plt.close()