...
def P_basic(in_dict):
    ...
    for vn in vn_list:
        ...
        cs = pfun.add_map_field(ax, ds, vn, {'salt':(0,30), 'temp':(5,15)},#pinfo.vlims_dict,
...
def P_oae(in_dict):
    ...
    for vn in vn_list:
        if in_dict['auto_vlims']:
            pinfo.vlims_dict[vn] = ()
        ax = fig.add_subplot(1, len(vn_list), ii)
        cs = pfun.add_map_field(ax, ds, vn, {'alkalinity':(1000,2400), 'TIC':(0,3000)},#pinfo.vlims_dict,
                cmap=pinfo.cmap_dict[vn], fac=pinfo.fac_dict[vn], vlims_fac=pinfo.range_dict[vn])
        fig.colorbar(cs)
        # pfun.add_coast(ax)
        # ax.axis(pfun.get_aa(ds))
...
def P_sect_hcal(in_dict):
    ...
    if True:
        ...
        lat = G['lat_rho']
        zdeep = -175
        y_e = np.linspace(47.1, 48, 500)
        x_e = -122.6 * np.ones(y_e.shape)
    # or read in a section (or list of sections)
    else:
        tracks_path = Ldir['data'] / 'section_lines'
        tracks = ['Line_jdf_v0.p', 'Line_ps_main_v0.p']
...
def P_vertint_dye(in_dict):
    ...
    new_dye_vert_int_data = xr.DataArray(dye_vert_int, dims=('eta_rho', 'xi_rho'))
    ds['dye_01_vertint'] = new_dye_vert_int_data

    # PLOT CODE
    vn_list = ['dye_01_vertint']
    ii = 1
    for vn in vn_list:
        if in_dict['auto_vlims']:
            pinfo.vlims_dict[vn] = ()
        ...
        plt.locator_params(axis='x', nbins=3)
        pfun.dar(ax)
        ax.set_title('Total vertically integrated\ndye_01 [kg/m2]', fontsize=14)
        ax.set_xlabel('Longitude')
        if ii == 1:
            ax.set_ylabel('Latitude')
        elif ii == 2:
...
def P_sect_dye01(in_dict):
    ...
    # v2, v3, dist, idist0 = pfun.get_section(ds, vn, x, y, in_dict)
    # v2, v3, dist, idist0 = pfun.get_sect(in_dict['fn'], vn, x, y)
    ...
    # COLOR
    # scaled section data
    sf = pinfo.fac_dict[vn] * fld_s# v3['sectvarf']
    # now we use the scaled section as the preferred field for setting the
    # color limits of both figures in the case -avl True
    if in_dict['auto_vlims']:
        pinfo.vlims_dict[vn] = pfun.auto_lims(sf)
