...
def P_sect_upw_v_vel(in_dict):
    ...
    svlims = pinfo.vlims_dict[vn]
    cs = ax.pcolormesh(v3['distf'], v3['zrf'], sf,
                       vmin=svlims[0], vmax=svlims[1], cmap=cm.balance)#cmap=pinfo.cmap_dict[vn])
    fig.colorbar(cs, ax=ax)
    ax.set_xlabel('Distance (km)')
    ax.set_ylabel('Z (m)')
    ax.set_title('Section %s %s' % (pinfo.tstr_dict[vn],pinfo.units_dict[vn]))
    ...
    v_ana = [(F/f)*(1-np.exp(-1*(km*1000)/lambda_)) for km in kms]
    # plot interface (analytical model)
    ax.plot(kms,v_ana,color = 'xkcd:bubblegum pink',linewidth = 2,
