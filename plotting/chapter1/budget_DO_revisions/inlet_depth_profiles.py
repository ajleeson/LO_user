...
# # fig,axes = plt.subplots(3,7, figsize=(15,8.5), sharex=True)
# fig,axes = plt.subplots(3,5, figsize=(15,8.5),sharex=True)
# ax = axes.ravel()
...
#     for day in range(len(ds.ocean_time)):
#         depth = ds['z_rho'][day,:]
#         DO = ds['oxygen'][day,:] *32/1000 # [mg/L]
#         ax[i].plot(DO,depth,alpha=0.1,color='silver')
...
#             # calculate and plot interface location (inflection point)
#             # determine drho/dz
#             dDO_dz = np.diff(avg_DO)/np.diff(avg_depth)
#             minindx = dDO_dz.tolist().index(min(dDO_dz.tolist()))
#             interface_depth = np.nanmean(avg_depth[minindx:minindx+2])
#         else:
#             alpha=0.5
#             linewidth=2
#         ax[i].plot(avg_DO,avg_depth,linewidth=linewidth,color=colors[s],alpha=alpha)
#         ax[i].axhline(interface_depth,0,15,color='crimson',linewidth=1)
#         ax[i].text(np.nanmedian(avg_DO) - 1.3*(np.nanmax(avg_DO)-np.nanmin(avg_DO)),
#                    interface_depth+0.04*np.nanmin(avg_depth),
#                    str(round(interface_depth,1)) + ' m',color='crimson',ha='left',va='top',
#                    fontweight='bold')
