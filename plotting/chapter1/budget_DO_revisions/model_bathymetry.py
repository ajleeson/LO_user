...
plt.close('all')
fs = 10
pfun.start_plot(fs=fs, figsize=(12.3,8))
fig = plt.figure()
...
cbar_ax = fig.add_axes([0.117, 0.08, 0.515, 0.02])
...
ax0.text(-122.3328+0.02, 47.6061-0.25,'Seattle',color='darkred', rotation=90,
...
ax2.set_ylim([46.93,48.45])
ax2.set_yticklabels([])
ax2.set_xticklabels([])
# add title
ax2.set_title('(c) Terminal Inlets',fontsize=14,loc='left', fontweight='bold')# color='#EEEEEE')
...
for sta in sta_dict:
    ...
    if sta == 'elliot':
        sta = 'elliott'
    ax2.text(sta_lon+lon_off,sta_lat+lat_off,sta,va='center',ha=ha,color=color,
             fontsize = 12, rotation=rotation, fontweight='bold')
