...
out_dir = Ldir['LOo'] / 'pugetsound_DO' / 'figures' / 'twentyone' / 'depthVtime_property' / 'obsdiff'
...
for i,station in enumerate(sta_dict): # enumerate(['budd','penn']): #
    ...
    for j,vn in enumerate(vn_list):
        ...
        if station in ecol_stn:
            ...
            if j == 0:
                axis.set_title('Model - Observations')
            ...
            if vn == 'oxygen':
                # get data
                obs = df_ob_stn['DO (uM)']*pinfo.fac_dict['oxygen'] # convert to mg/L
                model = df_mo_stn['DO (uM)']*pinfo.fac_dict['oxygen'] # convert to mg/L
                var = model - obs
                print(var)
                z = df_ob_stn['z']
                ...
                cs = axis.scatter(time,z, c=var, vmin=vmin, vmax=vmax, cmap=cmap)
            if vn == 'phytoplankton':
                # get data
                obs = df_ob_stn['Chl (mg m-3)']
                model = df_mo_stn['Chl (mg m-3)']
                var = model - obs
                z = df_ob_stn['z']
                time = [pd.Timestamp(x) for x in df_ob_stn['time']]
                # get min and max bounds
            ...
            if vn == 'rho':
                ...
                # calculate density
                rho_obs = gsw.rho(df_ob_stn['SA'],df_ob_stn['CT'],p)
                rho_model = gsw.rho(df_mo_stn['SA'],df_mo_stn['CT'],p)
                var = rho_model - rho_obs
                 # get min and max bounds
                vmin = np.nanmin(var)
                vmax = np.nanmax(var)
                if vmin > 0 and vmax > 0:
                ...
                axis.text(0.05, 0.9, 'Model overestimates',color='crimson',
                        verticalalignment='bottom', horizontalalignment='left',
                        transform=axis.transAxes, fontsize=14)
                axis.text(0.95, 0.9, 'Model underestimates',color='royalblue',
