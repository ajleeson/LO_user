...
print('Tidal currents -------------------------')
for i,station in enumerate(sta_dict):

    # download .nc files
    fn = '../../../LO_output/extract/' + gtagex + '/moor/' + '/twentyoneinlets_older/' + jobname + '_hourly/' + station + '_' + startdate + '_' + enddate + '.nc'
    ds = xr.open_dataset(fn)
    ...
    if station in ['dabob','lynchcove','sinclair','carr','commencement']:
        # print('    {} = {}'.format(station,np.nanmean(U_rms)*100))
        print('    {} = {}'.format(station,np.nanmax(np.sqrt(ubar**2 + vbar**2)*100)))
