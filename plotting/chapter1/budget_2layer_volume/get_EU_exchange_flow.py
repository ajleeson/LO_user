"""
Calculate exchange flow terms for volume budget
...
out_dir = Ldir['LOo'] / 'pugetsound_DO' / ('VOLUME_budget_'+startdate+'_'+enddate) / '2layerEU_exchange_flow'
...
for i,station in enumerate(sta_dict): # enumerate(stations): 
    ...
    if math.isnan(z_interface):
        ...
        df['surface [m3/s]'] = exchange_surf_total
        df['deep [m3/s]'] = exchange_deep_total
        df['total [m3/s]'] = exchange_total
        # save to pickle file
