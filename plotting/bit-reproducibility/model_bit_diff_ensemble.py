...
From ipython: run model_bit_diff_ensemble
...
vn = 'NH4' #['NH4','NO3','oxygen','u','v','w','salt','temp'] # u, v, w, NO3, NH4 oxygen
date = '2012.10.07'

oceanhis = 'ocean_his_0002.nc'
...
no_WestPoint = 'cas7_newtraps00noWP_debugx11ab'
test_conditions_gtagex = ['cas7_newtraps00tinyWP_debugx11ab',
                          'cas7_newtraps001WP_debugx11ab',
                          'cas7_newtraps0010WP_debugx11ab',
                          'cas7_newtraps0050WP_debugx11ab',
                          'cas7_newtraps00_debugx11ab']
test_conditions_names = ['1E-6 mmol/m3',
                          '1% WP nutrients',
                          '10% WP nutrients',
                          '50% WP nutrients',
                          '100% WP nutrients']
...
# out_dir = Ldir['LOo'] / 'AL_custom_plots' / 'West_Point_test'
...
# Initialize figure
fig, ax = plt.subplots(len(dates),len(test_conditions_gtagex),
                       figsize=(13, 9), sharex=True, sharey=True)

# loop through dates and loop through models
for i_date, date in enumerate(dates):
    for i_gtagex, gtagex in enumerate(test_conditions_gtagex):

        model2 = gtagex

        # get model output
        fp_model1 = Ldir['roms_out'] / model1 / ('f'+date) / oceanhis
        fp_model2 = Ldir['roms_out'] / model2 / ('f'+date) / oceanhis
        ...
        # plot
        ax[i_date,i_gtagex].pcolormesh(lon, lat, plot_data, cmap=cmap, norm=norm, shading='auto')
        ...
        ax[i_date,i_gtagex].scatter(WP_lon,WP_lat,s=8, facecolors='none', edgecolors='deeppink')

        # format figure
        # ax[i_date,i_gtagex].text(0.04, 0.95, 'Differences', color='black', fontweight='bold', fontsize=12,
        #         transform=ax[i_date,i_gtagex].transAxes)
        # ax[i_date,i_gtagex].text(0.04, 0.92, 'No Differences', color='turquoise', fontweight='bold', fontsize=12,
        #         transform=ax[i_date,i_gtagex].transAxes)
        # ax.set_title('{} and {} ({})'.format(model1,model2, oceanhis))
        # plt.xlabel('Lon', fontsize=12)
        # plt.ylabel('Lat', fontsize=12)
        pfun.dar(ax[i_date,i_gtagex])

        if i_gtagex == 0:
            ax[i_date,i_gtagex].set_ylabel('After {} day(s)'.format(days[i_date]))

        if i_date == 0:
            ax[i_date,i_gtagex].set_title(test_conditions_names[i_gtagex])

        # Salish Sea
        ax[i_date,i_gtagex].set_ylim([47,50])
        ax[i_date,i_gtagex].set_xlim([-126.5,-122])

        # # West Point
        # ax[i_date,i_gtagex].set_ylim([47.4,48])
        # ax[i_date,i_gtagex].set_xlim([-122.9,-122.1])
...
#     fig.suptitle('{} minus {}\n'.format(model1,model2) + date + ' {}'.format(oceanhis),
...
for i_date, date in enumerate(dates):
    for i_gtagex, gtagex in enumerate(test_conditions_gtagex):
        ...
        res = ds_model2[vn].squeeze() - ds_model1[vn].squeeze()  # shape (s_rho, eta_rho, xi_rho)
        ...
        # Residual
        ax[i_date,i_gtagex].scatter(distance, residuals, s=5, alpha=0.3,color='black',zorder=5)
        # format figure
        # ax[i_date,i_gtagex].set_ylabel(f'{vn} residual {pinfo.units_dict[vn]}\n(West Point - no West Point)',fontsize=12)
        ax[i_date,i_gtagex].grid(True,color='gainsboro')
        # ax[i_date,i_gtagex].set_ylim([-0.2,3.6])
        ...
        if i_date == 2 and i_gtagex == 2:
            ax[i_date,i_gtagex].set_xlabel(f'Distance from West Point [km]',fontsize=12)


plt.suptitle(f'Residuals of {vn} {pinfo.units_dict[vn]} vs. distance from West Point',fontsize=14)
plt.tight_layout()
...
for i_date, date in enumerate(dates):
    for i_gtagex, gtagex in enumerate(test_conditions_gtagex):
        ...
        # Absolute value and log transform
        ax[i_date,i_gtagex].scatter(pos_distances, pos_residuals, s=5, alpha=0.05,color='royalblue',
                    label='Positive Residual',zorder=5)
        ax[i_date,i_gtagex].scatter(neg_distances, neg_residuals, s=5, alpha=0.05,color='crimson',
                    label='Negative Residual',zorder=5)
        # # format figure
        ax[i_date,i_gtagex].set_yscale('log')
        # ax[1].set_xlim([0,25])
        # ax[1].set_ylim([10e-12,10e1])
        # ax[1].set_xlabel(f'Distance from West Point [km]',fontsize=12)
        # ax[1].set_ylabel(f'{vn} residual {pinfo.units_dict[vn]}\n(West Point - no West Point)',fontsize=12)
        ax[i_date,i_gtagex].grid(True,color='gainsboro')
        if i_date == 0 and i_gtagex == 0:
            ax[i_date,i_gtagex].legend(loc='upper right',fontsize=12)
