...
fp = Ldir['data'] / 'grids' / 'cas7' / 'wwtp_info.csv'
griddf0_wwtps = pd.read_csv(fp)
...
# Ocean -------------------------------------------------------------
# Data from Mackas & Harrison (1997)
...
for j in range(1):
    ...
    for i,sname in enumerate(source_name):
        ...
        ax.set_title(letters[i]+' Average {} DIN Load'.format(sname), fontsize=20, loc='left')
        pfun.dar(ax)
        ...
        plt.savefig(out_dir / ('loading_plot.png'),transparent='True',bbox_inches='tight')
