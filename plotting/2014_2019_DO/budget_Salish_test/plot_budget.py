...
for i,station in enumerate(['jdf1_2014']): # enumerate(sta_dict):
        # print status
        print('({}/{}) Working on {}...'.format(i+1,len(sta_dict),station))

        z_interface = -20 # for Salish Sea
...
# ---------------------------------- plot and save --------------------------------------------
            ...
            # ax[0].plot(dates_local[1::],traps_surf,color=traps_color,linewidth=1,linestyle='--',label='TRAPS')
            ...
            # ax[1].plot(dates_local[1::],traps_deep,color=traps_color,linewidth=1,linestyle='--',label='TRAPS')
            ...
            print('    (?) WHY IS THERE SUCH A LARGE ERROR?? (vertical exchange are not equal and opposite)')
