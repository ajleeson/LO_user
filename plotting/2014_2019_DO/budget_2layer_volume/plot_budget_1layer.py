...
out_dir = Ldir['LOo'] / 'pugetsound_DO' / ('VOLUME_budget_'+startdate+'_'+enddate) / '1layer_figures'
...
##########################################################
            ...
            ax[0].set_title('(a) Budget'.format(-1*z_interface),
                            loc='left')
            ax[1].set_title(r'(b) Error [sum of surface and bottom vertical exchange]',
                            loc='left')
...
# ------------------------------- get rivers and WWTPs ----------------------------------------
            ...
            traps_all = traps_surf + traps_deep
...
# ------------------------------- get volume storage ----------------------------------------
            ...
            dVdt_all = np.diff(surf_V+deep_V)* (1/24) * (1/60) * (1/60)
            dVdt_color = 'deeppink'
...
# ---------------------------------- plot and save --------------------------------------------
            ...
            ax[0].plot(dates_local_daily[1:-1],TEF_all,color=TEF_color,
                       linewidth=1,linestyle='--',label='TEF Exchange Flow')
            ax[0].plot(dates_local_daily[0:-1],dVdt_all,color=dVdt_color,
                       linewidth=1.5,zorder=5,
                       label=r'$\frac{\mathrm{dV}}{\mathrm{dt}}$')
            ax[0].plot(dates_local_daily[1:-1],traps_all,color=traps_color,
            ...
            ax[1].plot(dates_local_daily[1:-1],error_TEF,color='darkgreen',
