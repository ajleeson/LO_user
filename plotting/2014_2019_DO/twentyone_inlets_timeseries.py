...
for i,station in enumerate(sta_dict): # enumerate(['commencement']): #
    ...
    for i,vn in enumerate(vn_list):
        ...
        if vn in ['oxygen','delta_rho']:
                axis = ax[1]
                plotlabel = r'(b) Bottom DO and $\Delta\rho$'
        else:
                axis = ax[0]
        ...
        if vn == 'oxygen':
            ...
            # plot
            axis.plot(dates_local,val,linewidth=4,color=color,
                      alpha=0.8,label='Bottom DO')
            # format
            axis.set_ylim([0,12])
            axis.set_ylabel(r'Bottom DO [mg $L^{-1}$]',fontsize=ls+2,color=color)
        ...
        if vn == 'NO3':
            ...
            axis.set_yscale('log')
            axis.set_ylabel(r'Concentration [mmol N $m^{-3}$]',fontsize=ls+2)
        ...
        if vn == 'phytoplankton':
            ...
            # plot
            axis.plot(dates_local,val,linewidth=6,color=color,
                      alpha=0.5,label='phytoplankton (Surf {}m)'.format(str(round(d))))
    ...
    ax[0].legend(loc='lower center',fontsize=ls-1,ncol=3, handletextpad=0.2)
