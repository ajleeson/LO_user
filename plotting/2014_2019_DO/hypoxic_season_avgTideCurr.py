...
vmin = 0
vmax = 0.1
cmap = cmocean.tools.crop_by_percent(cmocean.cm.amp, 5, which='min')
...
for year in years:
    ...
    v = np.sqrt(ds['ubar'].values**2 + ds['vbar'].values**2)
...
# Add colormap title
plt.suptitle(start+' to '+end+' average tidal current [m/s]',
            fontsize=44, fontweight='bold', y=0.95)

# Generate plot
plt.tight_layout
plt.subplots_adjust(left=0.05, right=0.95, top=0.85, wspace=0.02)
plt.savefig(out_dir / ('Tcurrent_avg_'+ start + 'THRU'+end+'.png'))