...
out_dir = Ldir['LOo'] / 'pugetsound_DO' / ('OLD_DO_budget_'+startdate+'_'+enddate) / '2layer_figures'
...
# --------------------------- get exchange flow terms ----------------------------------------
            fn = Ldir['LOo'] / 'pugetsound_DO' / ('OLD_DO_budget_' + startdate + '_' + enddate) / 'DO_exchange_flow' / (station + '.p')
            ...
            # Godin filter
            exchange_surf = zfun.lowpass(exchange_surf_unfiltered, f='godin') 
            exchange_deep = zfun.lowpass(exchange_deep_unfiltered, f='godin')
            exchange_color = 'blue'
...
# ---------------------------------- get BGC terms --------------------------------------------
            bgc_dir = Ldir['LOo'] / 'pugetsound_DO' / ('OLD_DO_budget_' + startdate + '_' + enddate) / 'DO_bgc' / station
            # get months
            ...
            # apply Godin filter
            photo_surf = zfun.lowpass(photo_surf_unfiltered, f='godin')
            photo_deep = zfun.lowpass(photo_deep_unfiltered, f='godin')
            cons_surf   = zfun.lowpass(cons_surf_unfiltered, f='godin')
            cons_deep   = zfun.lowpass(cons_deep_unfiltered, f='godin')
            airsea_surf = zfun.lowpass(airsea_surf_unfiltered, f='godin')
            ddtDOV_surf = zfun.lowpass(ddtDOV_surf_unfiltered, f='godin')
            ddtDOV_deep = zfun.lowpass(ddtDOV_deep_unfiltered, f='godin')

# ------------------------------- get rivers and WWTPs ----------------------------------------
            fn = Ldir['LOo'] / 'pugetsound_DO' / ('OLD_DO_budget_' + startdate + '_' + enddate) / 'DO_traps' / (station + '.p')
...
# ---------------------------------- plot and save --------------------------------------------
            ...
            for axis in [ax[0],ax[1],ax[2]]:
                ylimval_TEF = np.nanmax(np.abs(exchange_surf))*1.1
                ylimval_EU = 0# np.nanmax(np.abs(EU_surf))*1.1
