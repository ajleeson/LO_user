...
# highlight some inlets
for stn,station in enumerate(['lynchcove','holmes','dabob']): # stations: 
    # get segment information
    seg_name = Ldir['LOo'] / 'extract' / 'tef2' / 'seg_info_dict_cas7_c21_traps00.p'
    seg_df = pd.read_pickle(seg_name)
    ji_list = seg_df[station+'_p']['ji_list']
...
# highlight some inlets
for stn,station in enumerate(['crescent']): # stations: 
    # get segment information
    seg_name = Ldir['LOo'] / 'extract' / 'tef2' / 'seg_info_dict_cas7_c21_traps00.p'
    seg_df = pd.read_pickle(seg_name)
    ji_list = seg_df[station+'_p']['ji_list']
...
ax.set_xlim(-123.29, -122.1) # Puget Sound
ax.set_ylim(46.95, 48.66) # Puget Sound
...
plt.savefig('simple_terminal_inlet_map.png',transparent=True)