...
for i,station in enumerate(['lynchcove']): # enumerate(sta_dict):
        ...
        # format figure
        plt.suptitle(station + ': TEF Volume Budget (Godin Filter)',size=14)
        ...
        ax[1].set_title(r'(b) Error = $\frac{\mathrm{dV}}{\mathrm{dt}}$ - (TEF exchange + TRAPS)', loc='left')
...
# --------------------------- get exchange flow terms ----------------------------------------
        ...
        Q_p = tef_df['q_p'] # Qout
        Q_m = tef_df['q_m'] # Qin
        exchange = Q_p.values + Q_m.values
...
# --------------------------- get EU exchange flow terms ----------------------------------------
        ...
        EU_exchange_unfiltered = df_exchange['total [m3/s]'].values 
        # 10-day hanning window filter (10 days = 240 hours)
        # EU_exchange = zfun.lowpass(EU_exchange_unfiltered, f='hanning', n=240)
        EU_exchange = zfun.lowpass(EU_exchange_unfiltered, f='godin')
        EU_exchange_color = 'blue'
...
# --------------------------- get storage terms ----------------------------------------
        ...
        dVdt = zfun.lowpass(dVdt_unfiltered, f='godin')[36:-34:24]
...
# ------------------------------- get rivers and WWTPs ----------------------------------------
        ...
        # 10-day hanning window filter (10 days = 240 hours)
        # traps = traps_unfiltered
        # traps = zfun.lowpass(traps_unfiltered, f='hanning', n=240)
        traps = zfun.lowpass(traps_unfiltered, f='godin')[36:-34:24]
        traps_color = 'turquoise'

# ---------------------------------- plot and save --------------------------------------------
        # plot budget
        ax[0].plot([dates_local[0],dates_local[-1]],[0,0],'k-',linewidth=0.5)
        ax[0].plot(dates_local,EU_exchange,color='deeppink',linewidth=2,alpha=0.5,label='EU Exchange Flow')
        ax[0].plot(dates_local_daily[1:-1],exchange,color=exchange_color,linewidth=1,linestyle='--',label='TEF Exchange Flow')
        ax[0].plot(dates_local_daily[1:-1],traps,color=traps_color,linewidth=3,alpha=0.6,label='TRAPS')
        ax[0].plot(dates_local_daily[1:-1],dVdt,color=dVdt_color,linewidth=2,alpha=0.6,
                    label=r'Storage = $\frac{\mathrm{dV}}{\mathrm{dt}}$')
        ax[0].legend(loc='best', ncol=2)
        
        # plot error
        print(np.nanmax(traps))
        error = dVdt - (exchange + traps)
        ax[1].plot([dates_local[0],dates_local[-1]],[0,0],'k-',linewidth=0.5)
        ax[1].plot(dates_local_daily[1:-1],error)
