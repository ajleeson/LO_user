...
n = len(gtagexes)
print(n)
# colors = cmocean.cm.thermal(np.linspace(0.1, 0.8, n))
...
for i,station in enumerate(sta_dict.keys()):
    ...
    for j,gtagex in enumerate(gtagexes):
        ...
        axN[2].text(0.02, 0.8, '(c) Difference (with N minus N-less)', fontweight='bold',
            verticalalignment='bottom', horizontalalignment='left',
        ...
        axN[j].tick_params(axis='y', labelcolor=phyto_color)
        axN[j].set_ylim([0,35])
        ...
        if j == 1:
            ...
            axN[j+1].tick_params(axis='y', labelcolor=phyto_color)
            # pick limits
        ...
        axDO[j].tick_params(axis='y', labelcolor=oxy_color)
        ...
        if j == 1:
            ...
            axDO[j+1].tick_params(axis='y', labelcolor=oxy_color)
            # pick limits
            lim = 1.2*(max(abs(min(diff)),max(diff)))
        ...
        axDO[2].set_xlabel('2013')
        axDO[2].tick_params(axis='x', labelrotation=30)
        axN[2].tick_params(axis='x', labelrotation=30)
