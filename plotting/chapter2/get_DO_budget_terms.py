...
#
        ...
        for station in sta_dict: # stations: 
            ...
            ret_DOvol = np.nansum(tmp_DOV)

            # get photosynthesis, nitrification, and respiration terms
            ret_photo = np.nansum(Oxy_pro[:,jj,ii])
            ret_nitri = np.nansum(Oxy_nitri[:,jj,ii])
            ret_respi = np.nansum(Oxy_remi[:,jj,ii])
            ...
            if cnt == 0:
                ...
                # get temp dataframe
                df_tmp = pd.DataFrame()
                df_tmp['DO*V [mmol]'] = [ret_DOvol]
                df_tmp['photo [mmol/hr]'] = [ret_photo]
                df_tmp['nitri [mmol/hr]'] = [ret_nitri]
                df_tmp['respi [mmol/hr]'] = [ret_respi]
