...
Testing on mac
run driver_roms00oaedye_ge -g cas7 -t t1dgeWB -x x11abd -r backfill -0 2020.01.01 -1 2020.01.02 -s continuation -np 192 -cpu cpu-g2 -grp macc -vip True -v True --get_forcing False --run_roms False --move_his False
...
while dt <= dt1:
    ...
    z_rho, z_w = zrfun.get_z(h, ds.zeta[0,:,:].to_numpy(), S) # depth of rho and w points. zero is the surface
    # get vertical thickness of all cells [m] 
    dzr = np.diff(z_w, axis=0) # [z,y,x]
    # get thicknesses at test cite
    dzr_site = dzr[:,iy,ix]
    
    # scale alkalinity by thickness of top 8 sigma layers
    nominal_thickness = 5 # [m] nominal thickness of sigma layers
    actual_thickness = np.nansum(dzr_site[-8:]) # thickness of top 8 sigma layers
    dalk_scaled = dalk * nominal_thickness/actual_thickness   

    # add alkalinity and save to a new file
    # In this case we are adding to the top 8 sigma layers in a single grid cell
    ds.alkalinity[0,-8:,iy,ix] += dalk_scaled

    # add the same amount of dye as alkalinity
    # conversion from alkalinity to dye, in units of (kg m-3 dye) / (mmol m-3 alkalinity)
    # assuming alkalinity is OH-
    alk2dye = 1.7e-5 
    ds.dye_01[0,-8:,iy,ix] += dalk_scaled*alk2dye
