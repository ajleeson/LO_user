...
ax.text(.05,0.1,'Surface 20 m',va='bottom',transform=ax.transAxes,fontweight='bold', fontsize = fs_label)
...
# depth cutoff of 20 m (looking at surface 20 m bottle data)
depth_cutoff = (depths <= 20)
x_obs_lon = x_obs_lon*depth_cutoff
x_obs_lon[x_obs_lon == 0] = 'nan'
y_obs_lat = y_obs_lat*depth_cutoff
y_obs_lat[y_obs_lat == 0] = 'nan'

# Plot data
newcmap = cmocean.cm.balance# cmocean.tools.crop(cmocean.cm.balance, vmin=vmin, vmax=vmax, pivot = 0)
map = plt.scatter(x_obs_lon,y_obs_lat, c=mod_minus_obs, cmap = newcmap, s = 200)
...
plt.savefig(out_dir / (ff_str + '_surface_diff_map' '.png'))
# plt.close('all')
