...
small = True#False # True for laptop size plot
...
if small:
    fs = 12
    pfun.start_plot(figsize=(11,8), fs=fs)
else:
    fs = 14
...
for depth_range in depth_list:
    ...
    for ii in range(len(vn_list)):
        ...
        ax.plot(x,y,marker='.',ls='',color=c_dict[depth_range], alpha=alpha, markeredgewidth=0)
        # print(len(x))
...
df_dict['obs'].plot(x='lon',y='lat',style='.',color='black',legend=False,
                    ax=ax, markersize=3)
# get the grid data
...
ax.pcolormesh(plon, plat, zm, linewidth=0.5, vmin=-1.5, vmax=0, cmap='Greys')
# pfun.add_coast(ax)
ax.axis([-130,-122,42,52])
pfun.dar(ax)
ax.set_xlabel('')
ax.set_ylabel('')
# ax.text(.05,0,f_str,va='bottom',fontsize=12,transform=ax.transAxes,fontweight='bold')
ax.text(.05,0,'Dividing depth = %d m\n' % (H),va='bottom',fontsize=11,transform=ax.transAxes,fontweight='bold')
