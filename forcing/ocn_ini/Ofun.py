...
def extrap_nearest_to_masked(X, Y, fld, i_inds, j_inds, fld0=0):
    """
    ...
    if fld.all() is np.ma.masked:
        ...
        # create masked array of new grid cells
        new_grid = np.empty(fld.shape)
        new_grid[:] = 0
        new_grid[i_inds,j_inds] = 1
        new_grid = ma.masked_equal(new_grid, 1)

        # get lat and lon of the new water cells
        xynew = np.array((X[new_grid.mask],Y[new_grid.mask])).T
        ...
        new_grid[i_inds,j_inds] = 1
        new_grid = ma.masked_equal(new_grid, 1)
        fldf[new_grid.mask] = fld[~fld.mask][aa]
        fldd = fldf.data
        checknan(fldd)
        return fldd
