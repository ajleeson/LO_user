...
if enable_pointsources == True:
    ...
    gri_df = pd.read_csv(gri_fn, index_col='rname')
    if Ldir['testing']:
        gri_df = gri_df.loc[['West Point', 'Birch Bay', 'Tacoma Central', 'US Oil & Refining', 'Oak Harbor Lagoon'],:]
    gri_df = gri_df.drop('Birch Bay') # Remove the Birch Bay treatment plant
    gri_df = gri_df.drop('Oak Harbor Lagoon') # Remove the Oak Harbor Lagoon treatment plant
