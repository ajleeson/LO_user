...
def start_ds(date,source,numdates,Nsources,source_type):
    ...
    ds = xr.Dataset(data_vars=dict(ID=(['source'], np.ones((Nsources,), dtype=int)),
        ...
        DON=(['source', 'date'], np.zeros((Nsources, numdates))),
        PON=(['source', 'date'], np.zeros((Nsources, numdates))),),
...
def add_metadata(ds):
    ...
    ds['DON'].attrs['long_name'] = 'dissolved organic nitrogen concentration'
    ds['DON'].attrs['units'] = 'mmol/m3'
    ds['PON'].attrs['long_name'] = 'particulate organic nitrogen concentration'
    ds['PON'].attrs['units'] = 'mmol/m3'
    return ds
...
def add_data(ds, source_ID, source_name, latlon_df, ecologydata_df):
    ...
    ds.NO3[i,:]  = ecologydata_df['NO3+NO2(mg/L)'] * 71.4 # convert to mmol/m3
    ds.NH4[i,:]  = ecologydata_df['NH4(mg/L)']     * 71.4 # convert to mmol/m3
    ds.DON[i,:]  = ecologydata_df['DON(mg/L)']     * 71.4 # convert to mmol/m3
    ds.PON[i,:]  = ecologydata_df['PON(mg/L)']     * 71.4 # convert to mmol/m3
...
out_fn = '../../../LO_data/' + trapsD + '/all_point_source_data_donpon.nc'
