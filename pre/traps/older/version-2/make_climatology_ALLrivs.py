"""
Note: this is not meant to be used to create climatology for LiveOcean
This is only meant to get a complete set of loading data for Puget Sound

Make climatologies for all rivers.
Discharge rate, temperature, and biogeochemisty variables.

Based on Ecology's timeseries, stored in LO_data/traps

To run, from ipython:
run make_climatology_ALLrivs.py
"""
...
clim_dir = Ldir['LOo'] / 'pre' / 'traps' / 'all_rivers' /'Data_historical'
...
riv_df = riv_singles_df.replace(' - 1', '', regex=True)
# get river names and river ids
rivnames = riv_df['Name'].values
rivids = riv_df['ID'].values
...
weird_rivers = ['Alberni Inlet', 'Brooks Peninsula', 'Campbell River', 'Chehalis R',
                'Clayoquot', 'Comox', 'Gold River', 'Holberg', 'Homathco River',
                'Klinaklini River', 'Knight Inlet', 'Neil Creek', 'Nimpkish River',
                'North East Vancouver Island', 'Owikeno Lake', 'Salmon River',
                'Seymour Inlet', 'Tahsis', 'Toba Inlet', 'Tsitika River',
                'Willamette R', 'Willapa R', 'Columbia R']
